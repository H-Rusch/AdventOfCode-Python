...
class Spiral:
    ...
    def spiral_movement(self):
        self.coordinate = Direction.next_position(
            self.direction, self.coordinate)
    ...
    def sum_all_adjacent(self):
        return sum(map(lambda coord: self.adjacent_sums[coord], get_all_adjacent(self.coordinate)))
...
def find_first_larger_than_input(n: int, spiral: Spiral) -> int:
    while True:
