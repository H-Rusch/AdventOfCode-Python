...
def part1():
    ...
    for entry in allergen_dict.items():
        allergen_dict[entry[0]] = list(functools.reduce(lambda a, b: set(a).intersection(b), entry[1]))
    ...
    important_ingredients = [ing for ing in set(important_ingredients) if ing not in allergen_ingredients]
...
def part2(allergen_dict: dict):
    print(",".join(allergen_dict[ing][0] for ing in sorted(allergen_dict)))


with open("input.txt", "r") as file:
    ingredient_list = [ingredients for ingredients in file.read().split("\n")]

    allergen_dictionary = part1()
    part2(allergen_dictionary)
