def part1(input):
    ...
    return timeslots[shortest_wait_index][0] * timeslots[shortest_wait_index][1]
...
def part2(input):
    ...
    M = 1
    equations = [
        [i, int(bus_list[i])] for i in range(len(bus_list)) if bus_list[i] != "x"
    ]
    for e in equations:
        M *= e[1]
...
def extended_euclidian_algorithm(a: int, b: int):
    ...
    table = [[None, None, a, b, 1, 0], [a, a // b, b, a % b, 0, 1]]
    j = 1
    while True:
        r_jm1 = table[j][2]
        r_j = table[j][3]
