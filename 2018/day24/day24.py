...
def part_1(immune: list, infection: list) -> int:
    copy_immune, copy_infection = deepcopy(immune), deepcopy(infection)

    fight(copy_immune, copy_infection)

    return sum(map(lambda g: g.units, copy_immune + copy_infection))


def part_2(immune: list, infection: list) -> int:
    boost = 1
    while True:
        copy_immune, copy_infection = deepcopy(immune), deepcopy(infection)
...
def get_all_targetings(attackers: list, defenders: list) -> list:
    attackers = sorted(attackers, key=lambda g: (g.calc_power(), g.initiative), reverse=True)
    to_attack = defenders[:]
    events = []
...
def parse_input():
    with open("input.txt", "r") as file:
        ...
        for line in lines:
            ...
            group = Group(units, hp, attack, attack_type, initiative, immunities, weaknesses)
            if team == 0:
...
if __name__ == "__main__":
    immune_army, infection_army = parse_input()

    print(f"Part 1: The winning army has {part_1(immune_army, infection_army)} units left.")

    print(
        f"Part 2: The immune system has {part_2(immune_army, infection_army)} units left when it wins for the first time.")
