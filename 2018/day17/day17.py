...
def part_1(coords: dict, min_y: int, max_y: int) -> int:
    return len([True for (x, y), tile in coords.items() if (tile == "|" or tile == "~") and min_y <= y <= max_y])


def part_2(coords: dict, min_y: int, max_y: int) -> int:
...
if __name__ == "__main__":
    clay_coordinates, lower_limit, upper_limit = parse_input()

    fill(clay_coordinates, upper_limit)

    print(f"Part 1: The water can reach {part_1(clay_coordinates, lower_limit, upper_limit)} tiles.")

    print(f"Part 2: There are {part_2(clay_coordinates, lower_limit, upper_limit)} water tiles left.")
