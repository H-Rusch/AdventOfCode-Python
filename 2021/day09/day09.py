...
def part_1(point_list: list, matrix: list) -> int:
    return sum(map(lambda n: n + 1, [matrix[y][x] for (x, y) in point_list]))


def part_2(point_list: list, matrix: list) -> int:
    basin_areas = sorted(get_basin_sizes(point_list, matrix), reverse=True)[:3]
...
if __name__ == "__main__":
    ...
    print(f"Part 1: The sum of the risk factor for all low points in the map is {part_1(low_points, heightmap)}.")

    print(f"Part 2: The size of the three largest basins multiplied together is {part_2(low_points, heightmap)}.")
