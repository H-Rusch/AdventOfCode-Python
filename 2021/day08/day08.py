def part_1(entry_list: list) -> int:
    count = 0
    for i in range(len(entry_list)):
        outputs = entry_list[i][1]
...
def part_2(entry_list: list) -> int:
    input_entries = [entry_list[i][0] for i in range(len(entry_list))]
    output_entries = [entry_list[i][1] for i in range(len(entry_list))]

    sum_outputs = 0
    for i in range(len(input_entries)):
        translation = deduction(input_entries[i])

        sum_outputs += int("".join([translate_digits(translation, entry) for entry in output_entries[i]]))

    return sum_outputs


def translate_digits(translation_dict: dict, randomized_entry: str) -> str:
    correct_letters = "".join(sorted([translation_dict[letter] for letter in randomized_entry]))
...
def deduction(input_entries: list):
    ...
    """
    input_entries = list(sorted(list(filter(lambda input_entry: len(input_entry) != 5, input_entries)), key=len))
    ...
    for i in [3, 4, 5]:
        ...
        if len(segment_set.intersection(letter_possibilities["c"])) == 1:  # missing segment c
            letter_to_set = "c"
        elif len(segment_set.intersection(letter_possibilities["e"])) == 1:  # missing segment e
            letter_to_set = "e"
...
def remove_option(possible_letters: dict, keys_to_filter: list, value_to_filter: set):
    for letter in possible_letters.keys():
        if all([letter != key for key in keys_to_filter]):
            possible_letters[letter] = possible_letters[letter].difference(value_to_filter)


def parse_input():
    with open("input.txt", "r") as file:
        data = file.read().splitlines()
        data = [[[pattern for pattern in part.split()] for part in entry.split("|")] for entry in data]
...
if __name__ == "__main__":
    ...
    print(f"Part 1: The digits 1, 4, 7 or 8 are {part_1(entries)} times in the output.")

    print(f"Part 2: The output values sum to: {part_2(entries)}.")
