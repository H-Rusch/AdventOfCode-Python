...
def parse_input():
    with open("input.txt", "r") as file:
        ...
        for line in file.read().splitlines():
            ...
            instructions.append((inst, int(x0), int(x1), int(y0), int(y1), int(z0), int(z1)))
...
if __name__ == "__main__":
    ...
    print(f"Part 1: In the small region there are {part_1(instruction_list)} cubes on.")

    print(f"Part 2: In the full region there are {part_2(instruction_list)} cubes on.")
